#!/usr/bin/env bash
# WallpaperApplicator.sh (Wayland)
...
# Usage: WallpaperApplicator.sh <random|relative/path/to/wallpaper.jpg> [sfw|nsfw]
...
displays=$(wayland-info 2>/dev/null | awk '
  ...
  ')
mapfile -t -O 1 displayNames < <(echo "$displays" | cut -d ':' -f1)
mapfile -t -O 1 displayRes < <(echo "$displays" | cut -d ':' -f2 | cut -d '@' -f1 | tr -d ' ')
mapfile -t -O 1 displayRefresh < <(echo "$displays" | cut -d '@' -f2 | tr -d ' ')
...
while [[ -n ${displayNames[$n]} ]]; do
    ...
    resolution=${displayRes[$n]}
    refreshRate=${displayRefresh[$n]}
